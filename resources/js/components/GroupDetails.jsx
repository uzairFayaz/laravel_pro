...
const GroupDetails = () => {
    ...
    const fetchGroupData = async () => {
        try {
            ...
            const [groupRes, membersRes, storiesRes, postsRes] =
                await Promise.all([
                    ...
                    axios.get(`/api/groups/${id}/stories`, {
                        headers: { Authorization: `Bearer ${token}` },
                    }),
                    axios.get(`/api/groups/${id}/posts`, {
                        headers: { Authorization: `Bearer ${token}` },
                    }),
                ]);
    ...
    const toggleSharing = async () => {
        try {
            ...
            setGroup((prev) => ({
                ...prev,
                is_shared: res.data.data.is_shared,
            }));
                ...
                stories.map((story) => (
                    <div key={story.id} className="border p-4 mb-2 rounded">
                        ...
                        <p className="text-sm text-gray-500">
                            Posted by: {story.user?.name} | Expires:{" "}
